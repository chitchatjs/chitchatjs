...
import "mocha";
import { AlexaBlock, AlexaBuilderContext, AlexaDialogContext, AlexaEvent, Locale, SlotType } from "../../../src/models";
...
describe("SlotTypeBlockBuilder", () => {
    describe("name, values, slotType validation tests", () => {
        it("should throw validation error if name, values and slotType all are missing", async () => {
            ...
            expect(err.message).to.be.equal("Either (name,values) or slotType is required in the SlotTypeBlock.");
        });
...
function assertSlotNameAndValues(
    ...
    if (types) {
        ...
        expect(JSON.stringify(types[0].values)).equals(
            JSON.stringify([{ name: { value: expectedValues[0] } }, { name: { value: expectedValues[1] } }])
        );
    }
}
