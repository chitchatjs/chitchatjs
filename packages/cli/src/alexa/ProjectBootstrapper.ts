...
// const CJS_METADATA_DIR = "cjs";
// const CJS_METADATA_FILE = "/cjs.json";
const ALEXA_OUT_DIR = "alexa";
...
export class ProjectBootstrapper {
    ...
    private bootstrapOutDir(outDir: string, currDir: string) {
        ...
        if (fs.existsSync(targetLocation)) {
            logger.info(
                `Output directory already exists. Skipping the bootstrapping.`
            );
            return;
        }
        ...
        let skillPackageTargetLocation = path.join(
            currDir,
            outDir,
            "/skill-package"
        );
        ...
        let deployerTargetLocation = path.join(
            currDir,
            outDir,
     ...
     */
    private bootstrapMetadata(
        outDir: string,
        currDir: string
    ): CjsMetadata | void {
        // Save default ask-resources.json
        let askResourcesTargetLocation = path.join(
            currDir,
            outDir,
            "/ask-resources.json"
        );
        ...
        fs.writeFileSync(
            ...
            JSON.stringify(defaultCjsMetadata.askResources, null, 2),
            "utf8"
        );

        // Save default ask-states.json
        let askStatesTargetLocation = path.join(
            currDir,
            outDir,
            "/.ask/ask-states.json"
        );
        ...
        fs.writeFileSync(
            ...
            JSON.stringify(defaultCjsMetadata.askStates, null, 2),
            "utf8"
        );
    }
}

// new ProjectBootstrapper().bootstrapProject({ outDir: "./out" } as BuildConfig);
